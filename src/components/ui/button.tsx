...
const buttonVariants = cva("inline-flex items-center justify-center !cursor-pointer gap-1 text-white", {
  variants: {
    variant: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, rounded, size, ...props }, ref) => {
    return (
      <button className={cn(buttonVariants({ variant, size, rounded, className }))} {...props}></button>
    );
  }
);
