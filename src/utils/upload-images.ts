...
export async function uploadImages(files: UploadFile[]): Promise<string[]> {
  ...
  const notEditedFiles = files.filter((file) => !file.originFileObj).map((item) => item.url!);
  ...
  const newFiles = await Promise.all(
    files
      ...
      .map(async (file) => {
        ...
        return result.cdnUrl ?? "https://images.unsplash.com/photo-1575936123452-b67c3203c357";
      })
  );
